...
//assumes num_samples is perfect square
...
void multiJitter(Vector2* samples, int num_samples) {
    ...
    //init points to the 'canonical' multi-jittered pattern
    for (int i = 0; i < sqrt_samples; i++) {
        for (int j = 0; j < sqrt_samples; j++) {
    ...
    for (int i = 0; i < sqrt_samples; i++) {
        for (int j = 0; j < sqrt_samples; j++) {
            ...
            t = samples[j*sqrt_samples + 1].y();
            samples[j*sqrt_samples + i].setY(samples[k*sqrt_samples + 1].y());
